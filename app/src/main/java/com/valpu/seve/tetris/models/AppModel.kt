...
class AppModel {
    ...
    fun generateField(action: String) {
        if (isGameActive()) {
            ...
            when (action) {
                Motions.LEFT.name -> coordinate?.x = currentBlock?.getPosition()?.x?.minus(1)
                Motions.RIGHT.name -> coordinate?.x = currentBlock?.getPosition()?.x?.plus(1)
                Motions.DOWN.name -> coordinate?.y = currentBlock?.getPosition()?.y?.plus(1)
                Motions.ROTATE.name -> {
    ...
    private fun blockAdditionPossible(): Boolean {
        ...
        if (!moveValid(currentBlock?.getPosition() as Point, currentBlock?.getFrameNumber()))
            return false
        return true
    }
    ...
    private fun validTranslation(position: Point, shape: Array<ByteArray>): Boolean {
        ...
        return if (position.y < 0 || position.x < 0) {
            false
        } else if (position.y + shape.size > FieldConstants.COLUMN_COUNT.value) {
            false
        } else if (position.x + shape[0].size > FieldConstants.ROW_COUNT.value) {
            false
        } else {
